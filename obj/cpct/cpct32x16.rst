...
Hexadecimal [16-Bits]
                             ...
                             24 
                             25 
   443D                      26 drawTilemap4x8_ag_32x16::
   ...
   443D FD 21 00 00   [14]   30     ld iy, #0000         ;; [4] IYL=View Window Width, IYH=View Window Height
                             31 
   4441                      32 nextRow:
                             33     ;; Disable interrupts and save SP before starting
   4441 F3            [ 4]   34     di                   ;; [1] Disable interrupts before starting (we are using SP to read values)
   4442 ED 73 BE 44   [20]   35     ld (restoreSP), sp   ;; [6] Save actual SP to restore it in the end
   ...
   4446 1A            [ 7]   40     ld     a, (de)    ;; [2] A = present tile-ID of the tile to be drawn
   4447 47            [ 4]   41     ld     b, a       ;; [1] B = A
                             42 
   ...
   4448 AF            [ 4]   49     xor    a       ;; [1] A = [00000000]
   4449 CB 38         [ 8]   50     srl    b       ;; [2] B = [0abcdefg] (Right shift, Carry = h)
   444B 1F            [ 4]   51     rra            ;; [1] A = [h0000000] (Rotate Right + Bit7 = h (Carry insertion))
   444C CB 38         [ 8]   52     srl    b       ;; [2] B = [00abcdef] (Right shift, Carry = g)
   444E 1F            [ 4]   53     rra            ;; [1] A = [gh000000] (Rotate Right + Bit7 = g (Carry insertion))
   444F CB 38         [ 8]   54     srl    b       ;; [2] B = [000abcde] (Right shift, Carry = f). 
   4451 1F            [ 4]   55     rra            ;; [1] A = [fgh00000] (Rotate Right + Bit7 = f (Carry insertion)). 
   4452 4F            [ 4]   56     ld     c, a    ;; [1] C = A. BC = 32*tile-ID complete.
...
Hexadecimal [16-Bits]
   ...
   4453 DD 21 00 00   [14]   62     ld    ix, #0000   ;; [4] IX = Pointer to start of tileset (0000 is placeholder set with tileset address)
   4457 DD 09         [15]   63     add   ix, bc      ;; [4] IX += Tile Offset  
   4459 DD F9         [10]   64     ld    sp, ix      ;; [3] Make SP Point to the start of the 32-byte screen pixel data
   ...
   445B C1            [10]    1    pop   bc             ;; [3] Get next 2 sprite bytes
   445C 71            [ 7]    2    ld  (hl), c          ;; [2] Copy byte 1
   445D 2C            [ 4]    3    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   445E 70            [ 7]    4    ld  (hl), b          ;; [2] Copy byte 2
   445F 2C            [ 4]    5    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4460 C1            [10]    6    pop   bc             ;; [3] Get next 2 sprite bytes
   4461 71            [ 7]    7    ld  (hl), c          ;; [2] Copy byte 3
   4462 2C            [ 4]    8    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4463 70            [ 7]    9    ld  (hl), b          ;; [2] Copy byte 4
   4464 CB DC         [ 8]   77     set    3, h       ;; [ 2] --000---=>--001--- (Next sprite line: 1)
   0029                      78     drawSpriteRow dec ;; [17] Copy tile line Right-to-Left [<<]
   4466 C1            [10]    1    pop   bc             ;; [3] Get next 2 sprite bytes
   4467 71            [ 7]    2    ld  (hl), c          ;; [2] Copy byte 1
   4468 2D            [ 4]    3    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4469 70            [ 7]    4    ld  (hl), b          ;; [2] Copy byte 2
   446A 2D            [ 4]    5    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   446B C1            [10]    6    pop   bc             ;; [3] Get next 2 sprite bytes
   446C 71            [ 7]    7    ld  (hl), c          ;; [2] Copy byte 3
   446D 2D            [ 4]    8    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   446E 70            [ 7]    9    ld  (hl), b          ;; [2] Copy byte 4
   446F CB E4         [ 8]   79     set    4, h       ;; [ 2] --001---=>--011--- (Next sprite line: 3)
   0034                      80     drawSpriteRow inc ;; [17] Copy tile line Left-to-Right [>>]
   4471 C1            [10]    1    pop   bc             ;; [3] Get next 2 sprite bytes
   4472 71            [ 7]    2    ld  (hl), c          ;; [2] Copy byte 1
   4473 2C            [ 4]    3    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4474 70            [ 7]    4    ld  (hl), b          ;; [2] Copy byte 2
   4475 2C            [ 4]    5    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4476 C1            [10]    6    pop   bc             ;; [3] Get next 2 sprite bytes
   4477 71            [ 7]    7    ld  (hl), c          ;; [2] Copy byte 3
   4478 2C            [ 4]    8    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4479 70            [ 7]    9    ld  (hl), b          ;; [2] Copy byte 4
   447A CB 9C         [ 8]   81     res    3, h       ;; [ 2] --011---=>--010--- (Next sprite line: 2)
   003F                      82     drawSpriteRow dec ;; [17] Copy tile line Right-to-Left [<<]
   447C C1            [10]    1    pop   bc             ;; [3] Get next 2 sprite bytes
   447D 71            [ 7]    2    ld  (hl), c          ;; [2] Copy byte 1
ASxxxx Assembler V02.00 + NoICE + SDCC mods  (Zilog Z80 / Hitachi HD64180), page 97.
Hexadecimal [16-Bits]



   447E 2D            [ 4]    3    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   447F 70            [ 7]    4    ld  (hl), b          ;; [2] Copy byte 2
   4480 2D            [ 4]    5    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4481 C1            [10]    6    pop   bc             ;; [3] Get next 2 sprite bytes
   4482 71            [ 7]    7    ld  (hl), c          ;; [2] Copy byte 3
   4483 2D            [ 4]    8    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4484 70            [ 7]    9    ld  (hl), b          ;; [2] Copy byte 4
   4485 CB EC         [ 8]   83     set    5, h       ;; [ 2] --010---=>--110--- (Next sprite line: 6)
   004A                      84     drawSpriteRow inc ;; [17] Copy tile line Left-to-Right [>>]
   4487 C1            [10]    1    pop   bc             ;; [3] Get next 2 sprite bytes
   4488 71            [ 7]    2    ld  (hl), c          ;; [2] Copy byte 1
   4489 2C            [ 4]    3    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   448A 70            [ 7]    4    ld  (hl), b          ;; [2] Copy byte 2
   448B 2C            [ 4]    5    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   448C C1            [10]    6    pop   bc             ;; [3] Get next 2 sprite bytes
   448D 71            [ 7]    7    ld  (hl), c          ;; [2] Copy byte 3
   448E 2C            [ 4]    8    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   448F 70            [ 7]    9    ld  (hl), b          ;; [2] Copy byte 4
   4490 CB DC         [ 8]   85     set    3, h       ;; [ 2] --110---=>--111--- (Next sprite line: 7)
   0055                      86     drawSpriteRow dec ;; [17] Copy tile line Right-to-Left [<<] 
   4492 C1            [10]    1    pop   bc             ;; [3] Get next 2 sprite bytes
   4493 71            [ 7]    2    ld  (hl), c          ;; [2] Copy byte 1
   4494 2D            [ 4]    3    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4495 70            [ 7]    4    ld  (hl), b          ;; [2] Copy byte 2
   4496 2D            [ 4]    5    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   4497 C1            [10]    6    pop   bc             ;; [3] Get next 2 sprite bytes
   4498 71            [ 7]    7    ld  (hl), c          ;; [2] Copy byte 3
   4499 2D            [ 4]    8    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   449A 70            [ 7]    9    ld  (hl), b          ;; [2] Copy byte 4
   449B CB A4         [ 8]   87     res    4, h       ;; [ 2] --111---=>--101--- (Next sprite line: 5)
   0060                      88     drawSpriteRow inc ;; [17] Copy tile line Left-to-Right [>>]
   449D C1            [10]    1    pop   bc             ;; [3] Get next 2 sprite bytes
   449E 71            [ 7]    2    ld  (hl), c          ;; [2] Copy byte 1
   449F 2C            [ 4]    3    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   44A0 70            [ 7]    4    ld  (hl), b          ;; [2] Copy byte 2
   44A1 2C            [ 4]    5    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   44A2 C1            [10]    6    pop   bc             ;; [3] Get next 2 sprite bytes
   44A3 71            [ 7]    7    ld  (hl), c          ;; [2] Copy byte 3
   44A4 2C            [ 4]    8    inc    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   44A5 70            [ 7]    9    ld  (hl), b          ;; [2] Copy byte 4
   44A6 CB 9C         [ 8]   89     res    3, h       ;; [ 2] --101---=>--100--- (Next sprite line: 4)
   006B                      90     drawSpriteRow dec ;; [17] Copy tile line Right-to-Left [<<]
   44A8 C1            [10]    1    pop   bc             ;; [3] Get next 2 sprite bytes
   44A9 71            [ 7]    2    ld  (hl), c          ;; [2] Copy byte 1
   44AA 2D            [ 4]    3    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   44AB 70            [ 7]    4    ld  (hl), b          ;; [2] Copy byte 2
   44AC 2D            [ 4]    5    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   44AD C1            [10]    6    pop   bc             ;; [3] Get next 2 sprite bytes
   44AE 71            [ 7]    7    ld  (hl), c          ;; [2] Copy byte 3
   44AF 2D            [ 4]    8    dec    l             ;; [1] HL++ / HL-- (4-byte aligned) -> next video mem location
   44B0 70            [ 7]    9    ld  (hl), b          ;; [2] Copy byte 4
   44B1 CB AC         [ 8]   91     res    5, h       ;; [ 2] --100---=>--000--- (Next sprite line: 0)
                             92 
...
Hexadecimal [16-Bits]
   ...
   44B3 2C            [ 4]  101     inc    l  ;; [1] /
   44B4 2C            [ 4]  102     inc    l  ;; [1] | HL+=3 (Incrementing L only)  
   44B5 2C            [ 4]  103     inc    l  ;; [1] \ 
   44B6 23            [ 6]  104     inc   hl  ;; [2] HL++ (HL += 4 in total)
                            105 
   ...
   44B7 13            [ 6]  108     inc   de           ;; [2] ++DE (Make tilemapPtr point to next tile to be drawn)
   007B                     109     dec__iyl           ;; [2] --IYL (--Width, One less tile to be drawn in this row)
   44B8 FD 2D                 1    .dw #0x2DFD  ;; Opcode for dec iyl
   44BA C2 46 44      [10]  110     jp    nz, nexttile ;; [3] if (IYL!=0), then more tiles are left to be drawn in this row,
   ...
   44BD 31 00 00      [10]  118     ld    sp, #0000      ;; [3] Restore SP (#0000 is a placeholder)
                     0083   119 restoreI = .
   44C0 FB            [ 4]  120     ei                   ;; [1] Restore previous interrupt status (Enabled or disabled)
   ...
   44C1 FD 25                 1    .dw #0x25FD  ;; Opcode for dec iyh
   44C3 28 10         [12]  126     jr     z, return     ;; [2/3] if (Height==0) then return
                            127 
                            128     ;;
   ...
   44C5 01 00 00      [10]  140     ld    bc, #0000      ;; [3] BC = (0x50 - 4*width) (#0000 is a placeholder that gets the value)
   44C8 09            [11]  141     add   hl, bc         ;; [3] HL += 0x50 - 4*width
                            142 
   ...
   44C9 FD 2E 00              1    .db #0xFD, #0x2E, #00  ;; Opcode for ld iyl, Value
...
Hexadecimal [16-Bits]
   ...
   44CC EB            [ 4]  156     ex    de, hl         ;; [1] Temporarily exchange HL<=>DE to do 16-bit maths for updating DE
                     0091   157 updateWidth = .+1
   44CD 01 00 00      [10]  158     ld    bc, #0000      ;; [3] BC = tilemapWidth - width
   44D0 09            [11]  159     add   hl, bc         ;; [3] HL += tilemapWidth - width (TilemapPtr points to first tile of next row)
   44D1 EB            [ 4]  160     ex    de, hl         ;; [1] Restore DE,HL into its proper registers, now with DE incremented
                            161 
   44D2 C3 41 44      [10]  162     jp    nextRow        ;; [3] Next Row
                            163 
   ...
   44D5 C9            [10]  167     ret
                            168 
   44D6                     169 setDrawTileMap4x8_ag_32x16::
                            170    ;; Set (tilesetPtr) placeholder
   44D6 22 55 44      [16]  171    ld (tilesetPtr), hl     ;; [5] Save HL into tilesetPtr placeholder
                            172 
   ...
   44D9 ED 43 3F 44   [20]  177    ld (widthHeightSet), bc ;; [6]
   44DD 79            [ 4]  178    ld     a, c                    ;; [1]
   44DE 32 CB 44      [13]  179    ld (restoreWidth), a    ;; [4] Set restore width after each loop placeholder
                            180    
   ...
   44E1 3D            [ 4]    3    dec    a          ;; [1] --A (In case A == RL, inverse subtraction should produce carry not to decrement RH)
   44E2 93            [ 4]    4    sub   e          ;; [1] A' = A - 1 - RL
   44E3 38 01         [12]    5    jr     c, 10000$  ;; [2/3] If A <= RL, Carry will be produced, and no decrement of RH is required, so jump over it
   44E5 15            [ 4]    6      dec   d        ;; [1] --RH (A > RL, so RH must be decremented)
   44E6                       7 10000$:   
                              8    ;; Now invert A to get the subtraction we wanted 
                              9    ;; { RL' = -A' - 1 = -(A - 1 - RL) - 1 = RL - A }
   44E6 2F            [ 4]   10    cpl            ;; [1] A'' = RL - A (Original subtraction we wanted, calculated trough one's complement of A')
   44E7 5F            [ 4]   11    ld    e, a    ;; [1] Save into RL (RL' = RL - A)
   44E8 ED 53 CE 44   [20]  187    ld (updateWidth), de   ;; [6] set the difference in updateWidth placeholder
                            188 
...
Hexadecimal [16-Bits]
   ...
   44EC 79            [ 4]  198    ld     a, c                ;; [1] A = Width
   44ED 87            [ 4]  199    add    a                   ;; [1] A = 2*Width
   44EE 87            [ 4]  200    add    a                   ;; [1] A = 4*Width
   44EF 2F            [ 4]  201    cpl                        ;; [1] A = -4*Width - 1
   44F0 C6 41         [ 7]  202    add #0x40 + 1              ;; [2] A = -4*Width-1 + 0x50+1 = 0x50 - 4*Width
   44F2 32 C6 44      [13]  203    ld (incrementHL), a ;; [4] Set HL increment in its placeholder
                            204 
   ...
   44F5 ED 57         [ 9]  210    ld     a, i             ;; [3] P/V flag set to current interrupt status (IFF2 flip-flop)
   44F7 3E FB         [ 7]  211    ld     a, #opc_EI       ;; [2] A = Opcode for Enable Interrupts instruction (EI = 0xFB)
   44F9 EA FE 44      [10]  212    jp    pe, int_enabled   ;; [3] If interrupts are enabled, EI is the appropriate instruction
   44FC 3E F3         [ 7]  213      ld   a, #opc_DI       ;; [2] Otherwise, it is DI, so A = Opcode for Disable Interrupts instruction (DI = 0xF3)
   44FE                     214 int_enabled:
   44FE 32 C0 44      [13]  215    ld (restoreI), a ;; [4] Set the Restore Interrupt status at the end with corresponding DI or EI
                            216 
   4501 C9            [10]  217    ret                     ;; [3] Return to caller
                            218 
                            219 
                            220 
   4502                     221 getScreenPtr_32x16::
                            222    ;; Extract line number (L) from Y-coordinate
                            223    ;; And multiply it by 256. rHL = 256 * L
   4502 78            [ 4]  224    ld    a, b     ;; [1] rA = Y-Coordinate
   4503 E6 07         [ 7]  225    and   #0x07    ;; [2] /
   4505 67            [ 4]  226    ld    h, a     ;; [1] \ rH = Y % 8      ;; << rH contains Line number [0-7] inside character Row
   ...
   4506 A8            [ 4]  232    xor   b        ;; [1] / rA = ( rB and #0x07 ) xor rB  =  rB and #0xF8
   4507 6F            [ 4]  233    ld    l, a     ;; [1] \ rL = 8*int(Y/8) ;; << L contains Screen Character Row multiplied by 8
   ...
   4508 29            [11]  242    add   hl, hl   ;; [3] / rHL' = 8*rHL
   4509 29            [11]  243    add   hl, hl   ;; [3] | rHL' = 2048*L + 64*R
   450A 29            [11]  244    add   hl, hl   ;; [3] \ 
...
Hexadecimal [16-Bits]
   ...
   450B 06 00         [ 7]  250    ld     b, #0   ;; [2] / As rC = X-Coordinate, having rB=0 makes rBC = X-Coordinate
   450D 09            [11]  251    add   hl, bc   ;; [3] \ rHL' = rHL + X
                            252 
                            253    ;; Add up screen start address we still keep in DE
   450E 19            [11]  254    add   hl, de   ;; [3] rHL' = rHL + screen_start
                            255 
                            256    ;; HL now contains the pointer to the byte in the video buffer. Just return it
   450F C9            [10]  257    ret            ;; [3] return rHL = Pointer to the video buffer at (X,Y) byte coordinates
                            258 
                            259 
                            260 
                            261 
                            ...
                            265 ;; B = Height
                            266 ;; C = Width
   4510                     267 drawSprite_32x16::
   4510 79            [ 4]  268    ld a, c
   4511 32 1C 45      [13]  269    ld (spr_width), a
   4514 ED 44         [ 8]  270    neg
   4516 32 25 45      [13]  271    ld (spr_width_neg), a   
   00DC                     272    ld__ixl_b ;; .db 0xDD 0x68
   4519 DD 68                 1    .dw #0x68DD  ;; Opcode for ld ixl, b
                            273 
   451B                     274 next_line:
                            275    ;; copy sprite row
                     00DF   276 spr_width = . +1
   451B 01 00 00      [10]  277    ld bc, #0   ;; 01 00 00
   451E ED B0         [21]  278    ldir        ;; *DE = *HL; ++HL; ++DE; --BC
                            279    
   00E3                     280    dec__ixl
   4520 DD 2D                 1    .dw #0x2DDD  ;; Opcode for dec ixl
   4522 C8            [11]  281    ret z
                            282    
                            283    ;; jump to next row
   4523 EB            [ 4]  284    ex de, hl
                     00E8   285 spr_width_neg = . +1
   4524 01 FC 07      [10]  286    ld bc, #0x7FC
   4527 09            [11]  287    add hl, bc
   4528 EB            [ 4]  288    ex de, hl
                            289    
                            290    ;; Detect jump outside character boundaries
   4529 3E 38         [ 7]  291    ld a, #0x38  ;;  0b00111000
   452B A2            [ 4]  292    and d
   452C 20 ED         [12]  293    jr nz, next_line
                            294 
                            295    ;; jump to the start of the next character
                            296 
   452E EB            [ 4]  297    ex de, hl
ASxxxx Assembler V02.00 + NoICE + SDCC mods  (Zilog Z80 / Hitachi HD64180), page 102.
Hexadecimal [16-Bits]



   452F 01 40 C0      [10]  298    ld bc, #-8*0x800 + 0x40
   4532 09            [11]  299    add hl, bc
   4533 EB            [ 4]  300    ex de, hl
                            301 
   4534 18 E5         [12]  302    jr next_line
                            303 
                            304 
                            ...
                            308 ;; C  = Width
                            309 ;; A  = Color
   4536                     310 drawSolidBox_32x16::
   4536 32 48 45      [13]  311     ld (sb_spr_color), a
                            312 
   4539 79            [ 4]  313     ld a, c
   453A 32 46 45      [13]  314     ld (sb_spr_width), a
   453D ED 44         [ 8]  315     neg
   453F 32 51 45      [13]  316     ld (sb_spr_width_neg), a   
   0105                     317     ld__ixl_b ;; .db 0xDD 0x68
   4542 DD 68                 1    .dw #0x68DD  ;; Opcode for ld ixl, b
                            318 
   4544                     319 sb_next_line:
                            320     ;; copy sprite row
                            321 
   4544 EB            [ 4]  322     ex de, hl
                     0109   323 sb_spr_width = . +1
   4545 3E 00         [ 7]  324     ld a, #0   ;; 01 00 00
                            325     
   4547                     326 sb_loop:
                     010B   327 sb_spr_color = . +1
   4547 36 00         [10]  328     ld (hl), #0
   4549 23            [ 6]  329     inc hl
   454A 3D            [ 4]  330     dec a
   454B 20 FA         [12]  331     jr nz, sb_loop
                            332    
   0110                     333     dec__ixl
   454D DD 2D                 1    .dw #0x2DDD  ;; Opcode for dec ixl
   454F C8            [11]  334     ret z
                            335    
                            336     ;; jump to next row
                     0114   337 sb_spr_width_neg = . +1
   4550 01 FC 07      [10]  338     ld bc, #0x7FC
   4553 09            [11]  339     add hl, bc
   4554 EB            [ 4]  340     ex de, hl
                            341    
                            342     ;; Detect jump outside character boundaries
   4555 3E 38         [ 7]  343     ld a, #0x38  ;;  0b00111000
   4557 A2            [ 4]  344     and d
   4558 20 EA         [12]  345     jr nz, sb_next_line
                            346 
                            347     ;; jump to the start of the next character
                            348 
   455A EB            [ 4]  349     ex de, hl
   455B 01 40 C0      [10]  350     ld bc, #-8*0x800 + 0x40
ASxxxx Assembler V02.00 + NoICE + SDCC mods  (Zilog Z80 / Hitachi HD64180), page 103.
Hexadecimal [16-Bits]



   455E 09            [11]  351     add hl, bc
   455F EB            [ 4]  352     ex de, hl
                            353 
   4560 18 E2         [12]  354     jr sb_next_line
                            355 
